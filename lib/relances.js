 ...
 * - prospect : 24h après visit_date, aucune vente
...
export function classifyRelance(lead, now = new Date()) {
  ...
  const t = now.getTime();
  const prospectDue = addDays(visit, 1).getTime();

  if (!lead.prospect_relance_at) {
    if (t < prospectDue) return { action: 'wait', reason: 'before_j1' };
    return { action: 'prospect', reason: 'j1_no_sale' };
  }
